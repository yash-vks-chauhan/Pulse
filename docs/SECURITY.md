...
| Unauthorized workspace access | With `PULSE_ACCESS_CODE` set, middleware gates every page and proxy route behind a signed httpOnly session cookie (HMAC-SHA256 over expiry + optional display name — tamper-evident, nothing sensitive inside). Login compares timing-safe and budgets 10 attempts / 15 min / IP (platform-set `x-real-ip` preferred over spoofable `x-forwarded-for`; the tracking map is size-capped and fails closed) | `apps/web/middleware.ts`, `apps/web/lib/auth.ts`, `apps/web/app/api/auth/login/route.ts` |
| Automated bots on the login form | Honeypot field invisible to humans; bots that auto-fill it get the same 401 as a wrong code (no CAPTCHA by design: the demo code is displayed for reviewers, and a third-party CAPTCHA script on the judges' critical path is a worse failure mode than the rate-limited threat it would mitigate) | `apps/web/app/login/login-form.tsx`, `apps/web/app/api/auth/login/route.ts` |
